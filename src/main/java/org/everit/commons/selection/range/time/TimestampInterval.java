...
public class TimestampInterval extends Interval<Timestamp> {

    private static final long serialVersionUID = -865088148345764928L;

    public TimestampInterval(final Timestamp lowerBound, final Timestamp higherBound) {
        super(lowerBound, higherBound);
    }

    public TimestampInterval(final Timestamp lowerBound, final Timestamp higherBound, final boolean lowerInclusive,
    ...
    protected long getHigherBoundInMillis() {
        return higherBound.getTime();
    }

    @Override
    protected long getLowerBoundInMillis() {
        return lowerBound.getTime();
    }
